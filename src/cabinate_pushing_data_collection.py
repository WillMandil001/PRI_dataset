...
from moveit_msgs.msg import CollisionObject, DisplayTrajectory, Constraints, OrientationConstraint, RobotTrajectory, TrajectoryConstraints, PositionConstraint
...
class FrankaRobot(object):
    ...
    def random_motion(self):
        for trial in range(self.num_trials):
            ...
            if cont == "n":
                ...
                # move to above camera pose.
                # task space info = [0.441462850535271, -0.01556462319792247, 0.3], [0.6599914188428677, 0.2825073939940094, -0.2670890711554867, 0.6428563816761805]
                # joint space info = [0.032609558091929704, 0.26340812214633874, 0.21259102998415752, -2.253307819263598, -1.409279827469779, 1.4070450107367496, 0.1613405828226608]
                self.move_to_joint_state([0.032609558091929704, 0.26340812214633874, 0.21259102998415752, -2.253307819263598, -1.409279827469779, 1.4070450107367496, 0.1613405828226608])

                # take image.


                # move down
                # task space info: ([0.47690252432211633, 0.006820855707207932, 0.05620921510932875], [0.6583875564370845, 0.27946286250434604, -0.2693012140784865, 0.6449055668123318])   
                # joint space info: [0.09292705353623941, 0.816399431479554, 0.1505256890581365, -2.060743617275305, -1.408644567523591, 1.4386793359447412, 0.5271093071418342]
                self.move_to_joint_state([0.09292705353623941, 0.816399431479554, 0.1505256890581365, -2.060743617275305, -1.408644567523591, 1.4386793359447412, 0.5271093071418342])

                # start data collection

                # push into the cabinate
                self.push_in_with_constraints(prev_position = [0.47690252432211633, 0.006820855707207932, 0.05620921510932875], prev_orientationxyzw = [0.6583875564370845, 0.27946286250434604, -0.2693012140784865, 0.6449055668123318],
                                           position = [0.47690252432211633, -0.3, 0.05], orientationxyzw = [0.6583875564370845, 0.27946286250434604, -0.2693012140784865, 0.6449055668123318])


                # # make pushing action
                self.push_across_with_constraints(prev_position = [0.47690252432211633, -0.3, 0.05], prev_orientationxyzw = [0.6583875564370845, 0.27946286250434604, -0.2693012140784865, 0.6449055668123318],
                                           position = [0.55, -0.3, 0.05], orientationxyzw = [0.6583875564370845, 0.27946286250434604, -0.2693012140784865, 0.6449055668123318])


                # stop data collection

    def push_across_with_constraints(self, position, orientationxyzw, prev_position=False, prev_orientationxyzw=False):
        constraints = Constraints()

        # ORIENTATION CONSTRAINTS:
        OC = OrientationConstraint()
        OC.header.frame_id = 'panda_link0'
        OC.orientation.x = orientationxyzw[0]
        OC.orientation.y = orientationxyzw[1]
        OC.orientation.z = orientationxyzw[2]
        OC.orientation.w = orientationxyzw[3]
        ...
        PC.weight = 1.0
        PC.target_point_offset.z = 0.01
        box = SolidPrimitive()
        box.type = SolidPrimitive.BOX
        box.dimensions = [1.0, 1.0, 0.095]
        PC.constraint_region.primitives = [box]

        mesh_pose = Pose()
        mesh_pose.position.x = prev_position[0]
        mesh_pose.position.y = prev_position[1]
        mesh_pose.position.z = prev_position[2]
        mesh_pose.orientation.x = 0
        mesh_pose.orientation.y = 0
        mesh_pose.orientation.z = 0
        mesh_pose.orientation.w = 1
        PC.constraint_region.primitive_poses = [mesh_pose]
        constraints.position_constraints = [PC]

        self.move_group.set_path_constraints(constraints)
        # PERFORM THE PUSHING ACTION
        start_pose = PoseStamped()
        start_pose.header.frame_id = '/panda_link0'
        start_pose.pose.position.x = position[0]
        start_pose.pose.position.y = position[1]
        start_pose.pose.position.z = position[2]
        start_pose.pose.orientation.x = orientationxyzw[0]
        start_pose.pose.orientation.y = orientationxyzw[1]
        start_pose.pose.orientation.z = orientationxyzw[2]
        start_pose.pose.orientation.w = orientationxyzw[3]
    ...
    def push_in_with_constraints(self, position, orientationxyzw, prev_position=False, prev_orientationxyzw=False):
        constraints = Constraints()
    ...
    def test_robot_motion(self, position, orientationxyzw):
        start_pose = PoseStamped()
        start_pose.header.frame_id = '/panda_link0'
        start_pose.pose.position.x = position[0]
        start_pose.pose.position.y = position[1]
